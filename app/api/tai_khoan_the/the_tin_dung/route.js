...
import checkLogicParams, { checkLogicParamsTheTinDung } from '../helpers'
...
export async function GET(request) {
  ...
  if (obj['offset'] && obj['limit']) {
    const { data, error } = await supabase.from('the_tin_dung').select('*').range(offset, limit)
    if (error) {
...
export async function POST(request) {
  try {
    ...
    */
    ...
    try {
      const checkLogic = checkLogicParamsTheTinDung(dataToCreate)
    } catch (error) {
      return error
    }

    const { error } = await supabase.from('the_tin_dung').insert(dataToCreate)
